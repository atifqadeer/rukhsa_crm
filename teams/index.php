<?php
    ...
    $query = "SELECT 
    teams.*, 
    users.first_name AS team_lead_fname, 
    users.last_name AS team_lead_lname, 
    COUNT(team_members.id) AS member_count
        FROM 
            teams 
        JOIN 
            users ON teams.team_lead_id = users.id
        LEFT JOIN 
            users AS team_members ON teams.id = team_members.team_id 
            AND team_members.id != teams.team_lead_id
        GROUP BY 
            teams.id
        ORDER BY 
            teams.id DESC;
    ...
    function softDeleteAgentById($id, $conn) {
        ...
        $stmt = $conn->prepare("UPDATE teams SET deleted_at = NOW() WHERE id = ?");
    ...
    $total_records_query = "SELECT COUNT(*) FROM `teams`";
                            ...
                            <div class="card">
                                ...
                                <div>
                                    ...
                                    <p class="text-muted font-size-13 mb-4">
                                        View and manage your teams efficiently.
                                    </p>
                                </div>
                                <div>
                                            ...
                                            if (mysqli_num_rows($result) > 0) {
                                                ...
                                                while ($row = mysqli_fetch_assoc($result)) { ?>
                                                    <tr>
                                                        ...
                                                        <td><?php echo htmlspecialchars($row['team_lead_fname'].' '.$row['team_lead_lname']); ?></td>
                                                        ...
                                                        <td>
                                                            <a href="/teams/edit.php?id=<?php echo $row['id']; ?>" class="btn btn-success waves-effect waves-light">Edit</a>
